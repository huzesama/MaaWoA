...
class dailyUniqueCompare(CustomRecognition):
	...
	def analyze(
		...
		global _daily_handling_count

		if _daily_handling_count <= 0:
			# 1. 解析参数 threshold
			try:
			...
			if not reco_detail or not reco_detail.hit or not reco_detail.best_result:
				return CustomRecognition.AnalyzeResult(  
					...
					detail={  
						...
						"raw_detail": reco_detail.raw_detail if reco_detail else None,  
					}  
				)
		...
		_daily_handling_count = _daily_handling_count - 1
		return CustomRecognition.AnalyzeResult(
			box=(0,0,1,1),
			detail={"count": _daily_handling_count}
		)
